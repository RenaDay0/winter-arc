...
const CATEGORIES = ['Здоровье','Спорт','Саморазвитие','Питание','Сон','Продуктивность','Другое']
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 18, fontWeight: 700, color: C.TEXT }}>Новая привычка</span>
          ...
          {goalType === 'numeric' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', gap: 16 }}>
                <div>
                  <label style={{ fontSize: 12, color: C.SECONDARY, display: 'block', marginBottom: 4 }}>Цель (число)</label>
                  <input
                    value={target}
                    onChange={e => setTarget(e.target.value)}
                    style={{
                      width: 80, background: C.CARD, border: `1.5px solid ${C.BORDER}`,
                      borderRadius: 8, color: C.TEXT, fontSize: 16, fontWeight: 700,
                      padding: '8px 10px', textAlign: 'center', outline: 'none', fontFamily: C.FONT,
                    }}
                    onFocus={e => (e.target.style.borderColor = C.ACCENT)}
                    onBlur={e  => (e.target.style.borderColor = C.BORDER)}
                  />
                ...
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: 12, color: C.SECONDARY, display: 'block', marginBottom: 4 }}>Единицы</label>
                  <input
                    value={unit}
                    ...
                    style={{
                      ...
                      borderRadius: 8, color: C.TEXT, fontSize: 14, padding: '8px 12px',
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6 }}>
                  {PRESETS.map(p => (
                    <button key={p.label} onClick={() => { setTarget(String(p.target)); setUnit(p.unit) }} style={{
                      background: C.BG, border: `1px solid ${C.BORDER}`, borderRadius: 8,
                      color: C.TEXT, padding: '4px 10px', fontSize: 12, cursor: 'pointer', fontFamily: C.FONT,
                    }}>
                      {p.label}
                    </button>
                  ))}
        ...
        <div style={{
          ...
          <button onClick={onClose} style={{
            ...
            color: C.TEXT, padding: '8px 20px', fontSize: 14, cursor: 'pointer', fontFamily: C.FONT,
          }}>Отмена</button>
          <button onClick={handleAdd} style={{
            background: C.ACCENT, border: 'none', borderRadius: 10,
            color: '#fff', padding: '8px 24px', fontSize: 14, fontWeight: 600,
